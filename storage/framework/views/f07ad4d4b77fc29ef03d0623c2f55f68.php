            ...
            <div class="flex h-16 shrink-0 items-center py-4">
                <div class="flex items-center space-x-3 min-w-0">
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-black p-1 overflow-hidden">
                        <img src="<?php echo e(asset('ini.jpg')); ?>" alt="Anugerah Rentcar Logo" class="h-full w-full object-contain">
                    </div>
                    <div class="min-w-0">
                        <h1 class="text-lg font-semibold text-gray-900 truncate"><?php echo e($settings->company_name ?? 'Anugerah Rentcar'); ?></h1>
                ...
                <ul role="list" class="space-y-1 px-2 py-2">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $menuItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(isset($item['children'])): ?>
                            ...
                            ?>
                            <li x-data="{ open: <?php echo e($isActive ? 'true' : 'false'); ?> }">
                                <button @click="open = !open" 
                                    ...
                                    <span class="truncate flex-1"><?php echo e($item['name']); ?></span>
                                    ...
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $item['children']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <li>
                                            <a href="<?php echo e(route($child['route'])); ?>" 
                                               wire:navigate
                                               class="group flex gap-x-3 rounded-md py-2 pl-8 pr-2 text-sm leading-6 text-gray-600 hover:bg-gray-50 hover:text-accent-500 <?php echo e(request()->routeIs($child['route'] . '*') ? 'bg-accent-50 text-accent-500 font-medium' : ''); ?>">
                        ...
                        <?php else: ?>
                            ...
                            <li>
                                <a href="<?php echo e(route($item['route'])); ?>" 
                                   wire:navigate
                                   class="group flex gap-x-3 rounded-md p-2 text-sm font-medium leading-6 text-gray-700 hover:bg-gray-50 hover:text-accent-500 <?php echo e(request()->routeIs($item['route'] . '*') ? 'bg-accent-50 text-accent-500' : ''); ?>">
                                    <?php echo $__env->make('components.icons.' . $item['icon'], ['class' => 'h-5 w-5 shrink-0'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
