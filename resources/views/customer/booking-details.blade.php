    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Nomor Booking: <span class="font-semibold text-gray-900">{{ $booking->booking_number }}</span></p>
            </div>
            <div>
                @php
                    $statusColors = [
                ...
                @endphp
                <span class="px-3 py-1 rounded-full text-sm font-medium {{ $statusColors[$booking->booking_status] ?? 'bg-gray-100 text-gray-800' }}">
                    ...
                    <div class="flex items-start space-x-4">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-2 space-y-1 text-sm text-gray-600">
                                <p><span class="font-medium">Tahun:</span> {{ $booking->car->year }}</p>
                                <p><span class="font-medium">Warna:</span> {{ $booking->car->color }}</p>
                                <p><span class="font-medium">Plat Nomor:</span> {{ $booking->car->license_plate }}</p>
                                <p><span class="font-medium">Transmisi:</span> {{ ucfirst($booking->car->transmission) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-900">{{ \Carbon\Carbon::parse($booking->start_date)->diffInDays($booking->end_date) }} Hari</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Dengan Driver</p>
                            <p class="font-semibold text-gray-900">{{ $booking->with_driver ? 'Ya' : 'Tidak' }}</p>
                        </div>
                        <div class="col-span-2">
                            <p class="text-sm text-gray-500">Lokasi Pengambilan</p>
                            <p class="font-semibold text-gray-900">{{ $booking->pickup_location }}</p>
                        </div>
                        <div class="col-span-2">
                            <p class="text-sm text-gray-500">Lokasi Pengembalian</p>
                            <p class="font-semibold text-gray-900">{{ $booking->return_location }}</p>
                        </div>
                        @if($booking->notes)
                            <div class="col-span-2">
        ...
        <div class="space-y-6">
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="border-t pt-3">
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-blue-600 text-lg">Rp {{ number_format($booking->total_amount, 0, ',', '.') }}</span>
                        </div>
                    ...
                    <div class="pt-3 border-t">
                        ...
                        <div class="text-center">
                            <span class="inline-block px-3 py-1 rounded-full text-sm font-medium {{ $paymentStatusColors[$booking->payment_status] ?? 'bg-gray-100 text-gray-800' }}">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 space-y-3">
                ...
                @if($booking->payment_status === 'pending')
                    <a href="{{ route('customer.bookings.payment', $booking) }}" 
                ...
                @if($booking->booking_status === 'pending')
                    <form method="POST" action="{{ route('customer.bookings.cancel', $booking) }}" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan pemesanan ini?')">
                        @csrf
                        @method('DELETE')
                ...
                <a href="{{ route('customer.bookings.ticket', $booking) }}" class="block w-full bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors font-medium text-center">
                    Download Tiket
                </a>

                <a href="{{ route('customer.bookings') }}" class="block w-full bg-gray-200 text-gray-700 py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors font-medium text-center">
                    Kembali ke Daftar
                </a>
            </div>
            ...
            <div class="bg-blue-50 rounded-lg p-6">
                ...
                <a href="{{ route('customer.support') }}" class="block w-full bg-white text-blue-600 py-2 px-4 rounded-lg hover:bg-gray-50 transition-colors font-medium text-center border border-blue-600">
                    Hubungi Dukungan
                </a>
            </div>
        </div>
