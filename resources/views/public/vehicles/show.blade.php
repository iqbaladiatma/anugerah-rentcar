                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="mb-8 animate-fade-in">
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            @if($car->photo_side || $car->photo_back)
                                <div class="grid grid-cols-2 gap-4">
                                    @if($car->photo_side)
                                        <img src="{{ asset('storage/' . $car->photo_side) }}" 
                                             alt="{{ $car->brand }} {{ $car->model }} - Side"
                                             class="w-full h-48 object-cover rounded-lg shadow-md">
                                    @endif
                                    @if($car->photo_back)
                                        <img src="{{ asset('storage/' . $car->photo_back) }}" 
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-8 border border-gray-100">
                        <div class="mb-6">
                            ...
                            <p class="text-lg text-gray-600">{{ $car->year }} • {{ $car->color }} • {{ $car->license_plate }}</p>
                        </div>
                                ...
                                @if($car->driver_fee_per_day > 0)
                                    <div class="mt-4 pt-4 border-t border-gray-200">
                                        <p class="text-sm text-gray-600">
                                            Driver fee: <span class="font-medium">Rp {{ number_format($car->driver_fee_per_day, 0, ',', '.') }}/day</span>
                                        </p>
                                    </div>
                                @endif
                            </div>
    ...
    <script>
        ...
        function updatePricing() {
            ...
            if (startDate && endDate && endDate > startDate) {
                ...
                const weeklyRate = {{ $car->weekly_rate ?? 0 }};
