    ...
    <section class="section-padding bg-primary-200">
        <div class="container-custom">
                ...
                @forelse($popularCars as $car)
                    <div class="card-hover overflow-hidden bg-white rounded-xl shadow-lg">
                        ...
                        <div class="p-6 lg:p-8">
                            <h3 class="text-xl font-semibold text-secondary-900 mb-2">{{ $car->brand }} {{ $car->model }}</h3>
                            <p class="text-secondary-600 mb-4 leading-relaxed text-sm">
                                {{ $car->year }} • {{ ucfirst($car->color) }} • {{ $car->license_plate }}
                            </p>
                            <div class="flex justify-between items-center">
                                <div>
                                    <span class="text-2xl lg:text-3xl font-bold text-accent-500">Rp {{ number_format($car->daily_rate, 0, ',', '.') }}</span>
                                    <span class="text-secondary-500 ml-1">/hari</span>
                                </div>
                                <a href="{{ route('vehicles.show', $car->id) }}" class="btn-outline text-sm">Lihat Detail</a>
                            </div>
                        </div>
                    </div>
                @empty
            ...
            <div class="text-center mt-12">
                <a href="{{ route('vehicles.catalog') }}" class="btn-primary text-lg">
    ...
    <section class="section-padding bg-gradient-to-r from-accent-500 to-accent-600 text-primary-500">
        <div class="container-custom text-center">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
                <a href="{{ route('vehicles.catalog') }}" class="bg-primary-500 text-accent-600 px-8 py-4 rounded-xl font-semibold hover:bg-primary-200 transition-all duration-200 hover:scale-105 shadow-medium text-lg">
                    Jelajahi Kendaraan
                </a>
                @guest('customer')
                    <a href="{{ route('customer.register') }}" class="border-2 border-primary-500 text-primary-500 px-8 py-4 rounded-xl font-semibold hover:bg-primary-500 hover:text-accent-600 transition-all duration-200 hover:scale-105 text-lg">
                        Daftar Sekarang
                    </a>
                @endguest
            </div>
