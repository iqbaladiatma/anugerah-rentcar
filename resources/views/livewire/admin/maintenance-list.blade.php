<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        ...
        <div class="bg-indigo-50 p-4 rounded-lg">
            ...
            <div class="text-lg font-bold text-indigo-900">{{ $statistics['inspection_count'] }} (Rp {{ number_format($statistics['inspection_cost'], 0, ',', '.') }})</div>
        </div>
    </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            <h3 class="text-lg leading-6 font-medium text-gray-900">
                Maintenance Records ({{ $maintenances->total() }} total)
            </h3>
        </div>
                        ...
                        @foreach($maintenances as $maintenance)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $maintenance->service_date->format('d M Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $maintenance->car->license_plate }}</div>
                                    <div class="text-sm text-gray-500">{{ $maintenance->car->brand }} {{ $maintenance->car->model }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
