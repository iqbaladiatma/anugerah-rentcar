                ...
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <div class="w-6 h-6 sm:w-8 sm:h-8 bg-gray-500 rounded-full flex items-center justify-center">
                            <span class="text-white text-xs sm:text-sm font-medium">{{ $statistics['total'] }}</span>
                        </div>
                    </div>
                    <div class="ml-3 sm:ml-5 min-w-0 flex-1">
                        <dl>
                            <dt class="text-xs sm:text-sm font-medium text-gray-500 truncate">Total</dt>
                            <dd class="text-sm sm:text-lg font-medium text-gray-900">{{ $statistics['total'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <div class="w-6 h-6 sm:w-8 sm:h-8 bg-yellow-500 rounded-full flex items-center justify-center">
                            <span class="text-white text-xs sm:text-sm font-medium">{{ $statistics['pending'] }}</span>
                        </div>
                    </div>
                    <div class="ml-3 sm:ml-5 min-w-0 flex-1">
                        <dl>
                            <dt class="text-xs sm:text-sm font-medium text-gray-500 truncate">Menunggu</dt>
                            <dd class="text-sm sm:text-lg font-medium text-gray-900">{{ $statistics['pending'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <div class="w-6 h-6 sm:w-8 sm:h-8 bg-blue-500 rounded-full flex items-center justify-center">
                            <span class="text-white text-xs sm:text-sm font-medium">{{ $statistics['confirmed'] }}</span>
                        </div>
                    </div>
                    <div class="ml-3 sm:ml-5 min-w-0 flex-1">
                        <dl>
                            <dt class="text-xs sm:text-sm font-medium text-gray-500 truncate">Dikonfirmasi</dt>
                            <dd class="text-sm sm:text-lg font-medium text-gray-900">{{ $statistics['confirmed'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <div class="w-6 h-6 sm:w-8 sm:h-8 bg-green-500 rounded-full flex items-center justify-center">
                            <span class="text-white text-xs sm:text-sm font-medium">{{ $statistics['active'] }}</span>
                        </div>
                    </div>
                    <div class="ml-3 sm:ml-5 min-w-0 flex-1">
                        <dl>
                            <dt class="text-xs sm:text-sm font-medium text-gray-500 truncate">Aktif</dt>
                            <dd class="text-sm sm:text-lg font-medium text-gray-900">{{ $statistics['active'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <div class="w-6 h-6 sm:w-8 sm:h-8 bg-gray-400 rounded-full flex items-center justify-center">
                            <span class="text-white text-xs sm:text-sm font-medium">{{ $statistics['completed'] }}</span>
                        </div>
                    </div>
                    <div class="ml-3 sm:ml-5 min-w-0 flex-1">
                        <dl>
                            <dt class="text-xs sm:text-sm font-medium text-gray-500 truncate">Selesai</dt>
                            <dd class="text-sm sm:text-lg font-medium text-gray-900">{{ $statistics['completed'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <div class="w-6 h-6 sm:w-8 sm:h-8 bg-red-500 rounded-full flex items-center justify-center">
                            <span class="text-white text-xs sm:text-sm font-medium">{{ $statistics['overdue'] }}</span>
                        </div>
                    </div>
                    <div class="ml-3 sm:ml-5 min-w-0 flex-1">
                        <dl>
                            <dt class="text-xs sm:text-sm font-medium text-gray-500 truncate">Terlambat</dt>
                            <dd class="text-sm sm:text-lg font-medium text-gray-900">{{ $statistics['overdue'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <button wire:click="toggleFilters" type="button" 
                    ...
                    <span class="hidden sm:inline">{{ $show_filters ? 'Sembunyikan Filter' : 'Tampilkan Filter' }}</span>
        ...
        @if($bookings->count() > 0)
            ...
            <div class="lg:hidden divide-y divide-gray-200">
                @foreach($bookings as $booking)
                    <div class="p-4 {{ $this->isOverdue($booking) ? 'bg-red-50' : 'hover:bg-gray-50' }}">
                        <div class="flex justify-between items-start mb-3">
                            <div class="flex-1 min-w-0">
                                <div class="font-medium text-sm text-gray-900 truncate">{{ $booking->booking_number }}</div>
                                <div class="text-xs text-gray-500">{{ $booking->created_at->format('d M Y') }}</div>
                            </div>
                            <div class="flex flex-col gap-1 ml-2">
                                <span class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-full {{ $this->getStatusBadgeClass($booking->booking_status) }}">
                                    {{ ucfirst($booking->booking_status) }}
                                </span>
                                <span class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-full {{ $this->getPaymentStatusBadgeClass($booking->payment_status) }}">
                                    {{ ucfirst($booking->payment_status) }}
                                </span>
                            </div>
                        </div>
                        <div class="space-y-1 text-xs text-gray-600">
                            <div><span class="font-medium">Pelanggan:</span> {{ $booking->customer->name }}</div>
                            <div><span class="font-medium">Telepon:</span> {{ $booking->customer->phone }}</div>
                            <div><span class="font-medium">Kendaraan:</span> {{ $booking->car->license_plate }} - {{ $booking->car->brand }} {{ $booking->car->model }}</div>
                            <div><span class="font-medium">Periode:</span> {{ $booking->start_date->format('d M Y') }} - {{ $booking->end_date->format('d M Y') }}</div>
                            <div><span class="font-medium">Jumlah:</span> {{ $this->formatCurrency($booking->total_amount) }}</div>
