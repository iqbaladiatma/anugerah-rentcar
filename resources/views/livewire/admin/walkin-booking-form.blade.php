                            ...
                            <div class="p-3 sm:p-4 space-y-3">
                                ...
                                <div class="flex rounded-lg bg-gray-100 p-1">
                                    <button type="button" 
                                            ...
                                            class="flex-1 py-2 px-3 text-xs sm:text-sm font-medium rounded-md transition-colors {{ $customer_mode === 'existing' ? 'bg-white text-green-700 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                                        Pelanggan Terdaftar
                                    </button>
                                    <button type="button" 
                                            wire:click="$set('customer_mode', 'new')"
                                            class="flex-1 py-2 px-3 text-xs sm:text-sm font-medium rounded-md transition-colors {{ $customer_mode === 'new' ? 'bg-white text-green-700 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                                        Buat Baru
                                    </button>
                                </div>
                                        ...
                                        <select wire:model.live="customer_id" 
                                            ...
                                            @foreach($customers as $customer)
                                                <option value="{{ $customer['id'] }}">
                                                    ...
                                                    @if($customer['is_member']) (Member) @endif
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        @if($customer_id)
                                            ...
                                            @if($selectedCustomer)
                                                <div class="mt-2 p-2 bg-green-50 rounded-lg text-xs text-green-800">
                                                    ...
                                                    <div>{{ $selectedCustomer['phone'] }} {{ $selectedCustomer['email'] ? '• ' . $selectedCustomer['email'] : '' }}</div>
                                ...
                                @else
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            @error('new_customer_name') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                        </div>
                                        <div>
                                            ...
                                            @error('new_customer_phone') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                        </div>
                                        <div>
                        ...
                        <div class="rounded-xl border border-gray-200 bg-white overflow-hidden">
                            ...
                            <div class="p-3 sm:p-4 space-y-3">
                                ...
                                <div>
                                    ...
                                    @error('car_id') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                    
                                    @if(!empty($availability))
                                        <div class="mt-1.5 flex items-center gap-1 text-xs {{ $availability['is_available'] ?? false ? 'text-green-600' : 'text-red-600' }}">
                                            ...
                                            @else
                                                ...
                                                <span>{{ $availability['error'] ?? 'Tidak tersedia' }}</span>
                                            @endif
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        @error('pickup_location') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        ...
                                        @error('return_location') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                                ...
                                <div class="p-3 sm:p-4">
                                    <div class="space-y-1.5 text-sm">
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-medium">{{ $pricing['duration_days'] ?? 0 }} hari</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Harga Dasar</span>
                                            <span>{{ $this->formatCurrency($pricing['base_amount'] ?? 0) }}</span>
                                        </div>
                                        @if(($pricing['driver_fee'] ?? 0) > 0)
                                            <div class="flex justify-between">
                                                <span class="text-gray-600">Sopir</span>
                                                <span>{{ $this->formatCurrency($pricing['driver_fee']) }}</span>
                                            </div>
                                        @endif
                                        @if($is_out_of_town && $out_of_town_fee > 0)
                                            <div class="flex justify-between">
                                                <span class="text-gray-600">Luar Kota</span>
                                                <span>{{ $this->formatCurrency($out_of_town_fee) }}</span>
                                            </div>
                                        @endif
                                        @if(($pricing['member_discount'] ?? 0) > 0)
                                            <div class="flex justify-between text-red-600">
                                                ...
                                                <span>- {{ $this->formatCurrency($pricing['member_discount']) }}</span>
                                            </div>
                                        @endif
                                        <div class="border-t border-green-300 pt-2 mt-2">
                                            <div class="flex justify-between">
                                                <span class="font-bold text-green-800">Total</span>
                                                <span class="font-bold text-green-800">{{ $this->formatCurrency($pricing['total_amount'] ?? 0) }}</span>
                                            </div>
                                            @if(($pricing['member_discount'] ?? 0) > 0)
                                                <p class="text-xs text-green-600 mt-1">
                                                    Hemat {{ $this->formatCurrency($pricing['member_discount']) }} dengan diskon member!
                                                </p>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="flex-shrink-0 rounded-b-2xl border-t border-gray-200 bg-gray-50 px-4 py-3 sm:px-6 sm:py-4">
                        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-3">
                            ...
                            <button wire:click="createBooking" type="button"
                                    ...
                                    @if(empty($availability['is_available'])) disabled @endif
